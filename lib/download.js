...
async function save(downloadQueue, progress = false) {
    var response = await axios.get(downloadQueue.url, { responseType: 'stream' });

    const dataSize = response.headers['content-length'];
    var loadedSize = 0;

    return new Promise((resolve, reject) => {
        var dest_stream = fs.createWriteStream(`${TEMP_PATH}${path.sep}${downloadQueue.filename}.${downloadQueue.extention}`);
        ...
        if (progress) {
            ...
            response.data.on('data', (data) => {
                ...
                var percentage = Math.round(loadedSize / dataSize * 100);
